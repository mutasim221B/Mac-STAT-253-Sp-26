---
title: "Classification - knn-trees"
...
Give a close look of the following R codes to find the closest neighbors (without `tidymodels`).
...
a.  regions are flexible / boundaries are "wiggly" (and the data viz showing the classification regions looks cool!) 
...
c.  When K is too big:  
  - the classification regions are very large / overly simple
...
b.  Among images classifies as tree:
    -   72% are actually trees (thus correctly classified)
    -   28% are actually grass (thus misclassified)
    -   0% are actually asphalt (thus misclassified)
...
c.  will vary (I personally like `min_n` in the 20--40 range)

d.  like pruning a real tree, tuning classification trees "lops off" branches

e.  they are boxy / less flexible. the vertical and horizontal boundaries between the regions reflect the binary splitting. 


<!-- note to self -->
<!-- I was surprised that the single-split tree (eg when min_n is high) predicted asphalt and tree, not asphalt and grass -->
<!-- we know from above grass is more common than tree -->
<!-- but apparently for NDVI > 0.08, tree becomes more common -->
<!-- hard to tell from scatterplot, but there are enough grass images with NDVI just below the cut-off that the tides turn in terms of which is more common above the cut-off -->
...
b.  The splits are sequential. We can't undo our first splits even if they don't end up being optimal later, and when we make the splits at any given point we're not thinking about how that will impact future splits. 

c.  Nope. Changing the scale of the predictors won't change our results.
