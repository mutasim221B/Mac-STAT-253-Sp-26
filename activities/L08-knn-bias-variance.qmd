...
The `tidymodels` KNN algorithm predicts $y$ using **weighted** averages. [kknn package documentation](https://www.rdocumentation.org/packages/kknn/versions/1.3.1/topics/kknn)
...
$$\sum_{i=1}^K w_i y_i$$
The [kernel wikipedia page](https://en.wikipedia.org/wiki/Kernel_(statistics)) shows common kernel functions.

The default "optimal" kernel used in `kknn` was defined in a [2012 Annals of Statistics paper](https://projecteuclid.org/journals/annals-of-statistics/volume-40/issue-5/Optimal-weighted-nearest-neighbour-classifiers/10.1214/12-AOS1049.full).
...
4. **Bias-Variance Tradeoff for Past Algorithms** 
...
### Part 1: Identifying neighborhoods {.unnumbered .smaller}
...
recipe_3_data
```
a. `Private_Yes` is now 0.6132441 or -1.6285680, not 1 or 0.
...
d. KNN always uses all of the predictors we give it. The concept of parsimony in the sense of simplicity/complexity of how many variables are included is not relevant here. Changing $K$ does not lead to a simpler/more complex model in the way changing $\lambda$ does for LASSO.
...
<details>
<summary>Solution</summary>
a. essentially none -- just that colleges with similar expenditures, enrollments, and school types (private vs public) would have similar graduation rates
...
d. Answers will vary. Come to office hours to discuss. 
</details>
<br> 
<br>
