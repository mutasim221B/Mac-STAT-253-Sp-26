...
> The output of the model is: one prediction for one observational unit. There is nothing to interpret (no plots, no coefficients, etc.). Also: there is no way to tune the model to use fewer predictors. See Exercises 5--6 from knn-bias-variance activities.
...
> The only assumption we made was that the outcome values of $y$ should be similar if the predictor values of $x$ are similar. No other assumptions are made. 
    ...
    - See Exercises 9--10 from knn-bias-variance activities for more discussion.
...
> Nothing. I'd say this is a con. There is nothing to interpret, so the model is more of a black box in terms of knowing why it gives you a particular prediction. See Exercises 7--9 from knn-bias-variance activities. Further details are in solutions for previous activity.
...
(a): Piecewise linear - corners at knots.
Not smooth; (b) Smoother - some continuity added; (c) More flexible curve; (d) Very smooth spline - typical cubic spline behavior.
...
**Part 2** (4--7): Solutions are not provided since these questions are part of HW3. 
